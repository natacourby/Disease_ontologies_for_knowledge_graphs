...
def build_graph(dict_inputs, ontology_name):
    global grakn_uri
...
def ontology_term_template(cross_reference, ontology_name):
    global dict_id
    x = ontology_name
...
# Input: line from "cross_reference.tsv" file
def ontology_term_source_template(cross_reference, ontology_name):
...
# Input: line from "cross_reference.tsv" file
def disease_template(cross_reference, ontology_name):
    global dict_id
    x = ontology_name
...
def main():
    dict_inputs = [
        {
            "data_path": data_folder + sys.argv[1] + "_additional_classes",
...
#        },
    ...
    build_graph(dict_inputs, sys.argv[1])
